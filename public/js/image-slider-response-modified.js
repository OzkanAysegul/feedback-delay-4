...
jsPsych.plugins['image-slider-response-modified'] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    if (trial.render_on_canvas) {
      ...
      // create html string with slider and labels, and add to slider container
      html = '<input type="range" class="jspsych-slider" value="' + trial.slider_start + '" min="' + trial.min + '" max="' + trial.max + '" step="' + trial.step + '" id="jspsych-image-slider-response-response" style="width: 600px;">';
      html += '<div>';
      ...
      for (var j = 0; j < trial.labels.length; j++) {
        ...
        html += '<div style="border: 1px solid transparent; display: inline-block; position: absolute; ' +
          'left:calc(' + percent_of_range + '% - (' + label_width_perc + '% / 2) - ' + offset + 'px); text-align: center; width: ' + label_width_perc + '%;">';
        html += '<span style="text-align: center; font-size: 16px; color: black;">' + trial.labels[j] + '</span>'; // Adjust font size and color
        html += '</div>';
      }
      html += '</div>';
      html += '</div>';
