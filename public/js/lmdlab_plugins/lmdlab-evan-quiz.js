 ...
 *
 * Shane Martin
 * edited by Evan to change how responses are output
 * 
 * renamed by Elliott & removed some saving
 * 
...
jsPsych.plugins['lmdlab-evan-quiz'] = (function() {
	var plugin = {};

	plugin.info = {
		name: 'lmdlab-evan-quiz',
		description: '',
		parameters: {
			questions: {
    ...
    // inject CSS for trial:  ADJUST text
    display_element.innerHTML = '<style id="jspsych-survey-multi-choice-css"></style>';
    var cssstr = ".jspsych-survey-multi-choice-question { margin-top: 2em; margin-bottom: 2em; font-size: 1em; text-align: left; }"+ // QUESTION formatting?
      ".jspsych-survey-multi-choice-text { font-size: 1.25em; font-weight: normal;} span.required {color: darkred;}"+ // LABEL formatting
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var j = 0; j < trial.questions[i].options.length; j++) {
        ...
        label.innerHTML = '&nbsp;&nbsp;'+ trial.questions[i].options[j]; // ADDED spaced indent before options
...
//         display_element.querySelector(question_selector + " p").innerHTML += "<span class='required'>*</span>";
    ...
    // add submit button
    trial_form.innerHTML += '<input type="submit" id="'+plugin_id_name+'-next" class="'+plugin_id_name+' lmdlab-btn"' + (trial.button_label ? ' value="'+trial.button_label + '"': '') + '></input>';
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var i=0; i<matches.length; i++){
        ...
        if(match.querySelector("input[type=radio]:checked") !== null){
          //console.log(match.querySelector("input[type=radio]:checked"))
          var val = match.querySelector("input[type=radio]:checked").value;
          var this_id = match.querySelector("input[type=radio]:checked").id;
          var chosen_idx = parseInt(this_id[this_id.length - 1]);
          choices.push(chosen_idx);
      ...
      jsPsych.pluginAPI.setTimeout(function (trialdata) {
      	end_trial(trial_data);//fill this place with function to call for object needed to appear after the delay
