...
jsPsych.plugins['lmdlab-survey-text'] = (function() {
	...
	plugin.trial = function(display_element, trial) {
		...
		for (var i = 0; i < trial.questions.length; i++) {
		  ...
		  if (trial.required==true) {
			  var minvar = "minlength=4";
		  } else {
			  var minvar = "minlength=0";
		  }
		  if (question.numbers == 1){      
			...
			html += '<textarea ' +minvar+ ' id="input-'+question_index+'" name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" cols="' + question.columns + '" rows="' + question.rows + '" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"></textarea>';
		  }
		  html += '</div>';
		}

		// add submit button
		html += '<input type="submit" id="jspsych-survey-text-next" class="lmdlab-btn jspsych-survey-text" value="'+trial.button_label+'"></input>';
		...
		display_element.querySelector('#jspsych-survey-text-form').addEventListener('submit', function(e) {
		  ...
		  jsPsych.pluginAPI.setTimeout(function (trialdata) {
			end_trial(trialdata);//fill this place with function to call for object needed to appear after the delay
		  }, trial.post_response_delay);

		});

		var end_trial = function(trialdata) {
