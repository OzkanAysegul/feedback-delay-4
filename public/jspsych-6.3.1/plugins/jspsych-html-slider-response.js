...
jsPsych.plugins['html-slider-response'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '">';
    html += '<input type="range" class="jspsych-slider" value="'+trial.slider_start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" id="jspsych-html-slider-response-response" style="width: 1000px;"></input><br>';
    html += '<div>'
    for(var j=0; j < trial.labels.length; j++){
      ...
      html += '<span style="text-align: center; font-size: 30px;">'+trial.labels[j]+'</span>'; // Adjust the font-size value (e.g., 16px)
      html += '</div>'
    }
    html += '</div>';
    html += '</div>';
